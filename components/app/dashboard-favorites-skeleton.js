...
export default function DashboardFavoritesSkeleton() {
  return (
    <div>
      <h2 className="animate-pulse w-24 h-3 mb-2 block font-medium bg-gray-200 rounded-xl text-sm font-medium text-gray-500">&nbsp;</h2>
      <ul role="list" className="mt-3 grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
        {projects.map((project) => (
          <li key={project.name} className="col-span-1 flex rounded-md shadow-sm">
            ...
            >
              <div className="h-5 w-5 bg-gray-200 rounded-full fill-yellow-100 text-yellow-400 animate-pulse" aria-hidden="true"></div>
            </div>
            <div className="flex flex-1 items-center justify-between truncate rounded-r-md border-t border-r border-b border-gray-200 bg-white">
              <div className="flex-1 truncate px-4 py-2 text-sm">
                <Link href={project.href} >
                  <a className="w-24 h-3 mb-2 block font-medium bg-gray-200 rounded-xl animate-pulse text-gray-900 hover:text-gray-600">
                    &nbsp;
                  </a>
                </Link>
                <p className="text-gray-500 bg-gray-200 h-4 rounded-xl animate-pulse">&nbsp;</p>
              </div>
